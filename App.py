...
if file is not None:
    ...
    with col2:  # Center column
        st.image(preprocessed_img_np, use_column_width=True)
...
''', unsafe_allow_html=True)
         ...
         <strong>Primary diagnosis confidence: {primary_prediction[1]*100:.1f}%</strong>
         </div>
         """, unsafe_allow_html=True)
         ...
         if secondary_prediction[1] > MODERATE_PROB_THRESHOLD:
             st.markdown(f"""
             **Secondary Consideration**: There is a {secondary_prediction[1]*100:.1f}% probability of {secondary_prediction[0]}
             """)
         ...
         if class_name_predicted != "No Tumour":
             with st.expander("Advanced Predictive Analysis", expanded=True):
                 ...
                 st.markdown("""
                 > **Important Medical Notice**: These predictions are based on statistical analysis and general patterns. They should not be used as the sole basis for medical decisions. Individual cases may vary significantly. Always consult with healthcare professionals for personalized medical advice.
         ...
         if primary_prediction[0] == 'Glioma':
             ...
             - **Urgency Level**: [CRITICAL] High - Immediate medical attention recommended
             ...
             - **Urgency Level**: [MODERATE] Moderate - Medical evaluation needed
             ...
             - **Status**: [NORMAL] No apparent tumor detected
